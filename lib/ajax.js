import { extend, generate_topic, post_string } from './util.js';
...
function Ajax(params) {
	var _params = extend({
		...
		topic = _params.url.indexOf('?') === -1 ? _params.url :
			_params.url.slice(0, _params.url.indexOf('?')),
		ni, t, _xhr,
		auth,
		path_args;
	...
	for (t in _mocks) {
		...
		if (!_mocks[t].methods.hasOwnProperty(_params.method)) {
			continue;
		}
		...
		create_async_callback(
			_mocks[t].methods[_params.method],
			path_args,
			_params
		);
	...
	if (_params.username || _params.password) {
		auth = (_params.username || '') + ':' + (_params.password || '');
	...
	// dont set this property, as it crashes older browsers
	// thanks to @digitalmaster
	...
	_xhr.send(_params.method === 'POST' ? post_string(_params.data) : null);
