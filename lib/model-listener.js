...
class ModelListener extends React.Component {
	...
	constructor(props) {
		...
		if (props.hasOwnProperty('model')) {
			if (props.model instanceof Model) {		// already initialized model
				this.model = props.model;
			} else if (props.model.prototype instanceof Model) {	// just a definition
				this.model = new props.model();
			} else {
				try {
					this.model = new this.constructor.defaultProps.model(props.model);	//just a json object
				} catch (e) {
